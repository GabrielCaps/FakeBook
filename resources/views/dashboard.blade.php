...
<div class="col-lg-9 mt-3 fs-5 overflow-auto row justify-content-center">
	<div class="container-fluid">
		<form class="row bg-white shadow rounded p-4" action="{{route ('post.create', app()->getLocale())}}" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
			@csrf
			<input class="placeholder border-1 col-md-12 rounded-pill shadow bg-white mt-1 p-2" type="text" name="content" placeholder="{{__ ('Write')}}">
			<div class="col-md-6">
...
fileChooser.onchange = e => {
    ...
    reader.readAsDataURL(fileToUpload);
			...
			<div class="d-flex justify-content-center">
				<button class="shadow col-md-4 btn btn-primary mt-2" type="submit">{{__ ('Create')}}</button>
			</div>
		</form>
		@if(session('msg'))
		 	<div class="row">
		 		<p class="col-md-12 text-center shadow mt-2 msg bg-info border-1 rounded-pill">{{session('msg')}}</p>
		 	</div>
	 	@endif 
	</div>
	 					...
	 					@if(Auth::user()->id == $Post->user_id)
							...
							<div class="col-3">
								<form action="{{route('post.destroy', ['id' => $Post->id], app()->getLocale())}}" method="POST">
						    		@csrf
						    		@method('DELETE')
						    		<button class="placeholder-sm bg-white border rounded-pill shadow w-100 mb-2">{{__ ('Delete')}}</button>		    		
						    	</form>
